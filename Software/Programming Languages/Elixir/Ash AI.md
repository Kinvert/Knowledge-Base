...
**MCP Overview**: Model Context Protocol (Anthropic) standardizes how LLMs access external data and tools. MCP servers expose resources and tools that LLM clients can use.

**MCP Server in Elixir**: Create MCP server to expose your application's resources:
```elixir
defmodule MyApp.MCPServer do
  ...
  resources do
    resource :users do
      ...
      list_handler fn ->
        ...
        {:ok, Enum.map(users, &serialize_user/1)}
      end
      
      read_handler fn uri ->
        user_id = extract_id(uri)
        user = MyApp.User |> Ash.get!(user_id)
        {:ok, serialize_user(user)}
      end
    end
  ...
  tools do
    tool :create_user do
      ...
      handler fn params ->
        ...
        |> Ash.Changeset.for_create(:create, params)
        |> Ash.create() do
          {:ok, user} -> {:ok, serialize_user(user)}
  ...
  # Expose prompts
  prompts do
    prompt :user_summary do
      description "Generate user summary"
      arguments do
        argument :user_id, :string, required: true
      end
      
      handler fn %{user_id: user_id} ->
        user = MyApp.User
        |> Ash.Query.load([:tasks, :projects])
        |> Ash.get!(user_id)
        
        {:ok, """
        User: #{user.name}
        Email: #{user.email}
        Active Tasks: #{length(user.tasks)}
        Projects: #{Enum.map_join(user.projects, ", ", & &1.name)}
        """}
      end
    end
  end
...
**MCP Client Configuration**: Connect to MCP servers (local or remote):
```elixir
config :ash_ai, :mcp_clients,
  local_server: [
    transport: :stdio,
    command: "node",
    args: ["./mcp-server.js"]
...
defmodule MyApp.MCPAgent do
  ...
  agent do
    ...
    # Enable MCP client
    mcp_client :local_server
    mcp_client :postgres_server
  end
end

# Agent can now use tools from MCP servers
# "Query the database for users created in the last week"
# Agent discovers available MCP tools and uses them
```

**Custom MCP Transport**: Implement custom transport for specialized MCP servers:
```elixir
defmodule MyApp.MCPTransport do
  @behaviour MCPServer.Transport
  
  def start_link(config) do
    # Connect to MCP server (HTTP, WebSocket, etc.)
  end
  ...
  def receive_response(conn) do
    # Receive and parse response
  end
end
```

**MCP Resource Sampling**: Let agents explore your data:
```elixir
resources do
  resource :documentation do
    ...
    read_handler fn uri ->
      ...
      content = File.read!("docs/#{extract_path(uri)}")
      {:ok, %{text: content}}
    end
  end
end

# Agent can read documentation to answer questions
# "According to our documentation, how do we handle user authentication?"
```
...
defmodule MyApp.Document do
  ...
  attributes do
    ...
    attribute :title, :string
    attribute :content, :string
    attribute :embedding, {:array, :float} # pgvector
  end
  
  embeddings do
    # Auto-generate embeddings for content
    embed_field :content do
      provider :openai
  ...
  actions do
    create :create do
      ...
      change AshAI.Embeddings.GenerateEmbedding
    end
  end
end
```
...
defmodule MyApp.SemanticSearch do
  def search(query, limit \\ 10) do
    ...
    {:ok, query_embedding} = AshAI.Embeddings.embed(
      query,
      provider: :openai,
      model: "text-embedding-3-small"
    )
    ...
    |> Ash.Query.sort([
      asc: fragment("embedding <-> ?::vector", ^query_embedding)
    ])
    |> Ash.Query.limit(limit)
    |> Ash.read!()
  end
...
defmodule MyApp.RAGAgent do
  ...
  agent do
    ...
    provided context. Only use information from the given context.
    If the answer isn't in the context, say you don't know.
    """
  end
  
  def answer_with_context(question) do
    # 1. Retrieve relevant documents
    relevant_docs = MyApp.SemanticSearch.search(question, limit: 5)
    
    # 2. Build context
    context = Enum.map_join(relevant_docs, "\n\n", fn doc ->
      """
      Document: #{doc.title}
      #{doc.content}
      """
    ...
    {:ok, response.content, relevant_docs}
  end
end

# Usage
{:ok, answer, sources} = MyApp.RAGAgent.answer_with_context(
...
defmodule MyApp.DocumentChunker do
  def chunk_document(document, chunk_size \\ 500, overlap \\ 50) do
    ...
    Enum.map(chunks, fn {chunk_text, index} ->
      MyApp.DocumentChunk
      |> Ash.Changeset.for_create(:create, %{
        ...
        chunk_index: index,
        metadata: %{
          start_char: calculate_start_position(index, chunk_size, overlap)
        }
      })
      |> Ash.create!()
    end)
  ...
  defp build_chunks(paragraphs, chunk_size, overlap) do
    # Implementation of overlapping chunking
    # Ensures semantic coherence at boundaries
  end
end
```
...
def hybrid_search(query, limit \\ 10) do
  ...
  keyword_results = MyApp.Document
  |> Ash.Query.filter(
    fragment("to_tsvector('english', content) @@ plainto_tsquery('english', ?)", ^query)
  )
  |> Ash.read!()
  
  # 2. Semantic search
  {:ok, embedding} = AshAI.Embeddings.embed(query)
  semantic_results = MyApp.Document
  |> Ash.Query.filter(
    fragment("embedding <-> ?::vector", ^embedding) < 0.5
  )
  |> Ash.read!()
...
defmodule MyApp.Reranker do
  ...
  def rerank(query, documents) do
    ...
    #{Enum.with_index(documents, 1) |> Enum.map_join("\n", fn {doc, i} ->
      "#{i}. #{doc.title}: #{String.slice(doc.content, 0, 200)}..."
    end)}
    
    Respond with just the numbers in order, comma-separated.
    """
    ...
    ranking = parse_ranking(response.content)
    Enum.map(ranking, fn idx -> Enum.at(documents, idx - 1) end)
  end
end
```
